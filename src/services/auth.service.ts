...
export class AuthService {
  ...
  async initializeOAuth(): Promise<void> {
    ...
    try {
      ...
      if (!config) {
        console.warn('OIDC configuration not available, using mock authentication');
      ...
      const useBackendTokenEndpoint = config.responseType === 'code' && !config.disablePKCE;
      ...
      if (config.showDebugInformation) {
        console.log('[AuthService] Configuring OAuth with:', {
          ...
          responseType: config.responseType,
          scope: config.scope,
          tokenEndpoint: authConfig.tokenEndpoint || 'default (from discovery document)',
          useBackendTokenEndpoint
        });
      }
      ...
      if (useBackendTokenEndpoint) {
        (this.oauthService as any).tokenEndpoint = `${window.location.origin}/api/oidc/token`;
        if (config.showDebugInformation) {
          console.log('[AuthService] Token endpoint overridden to:', `${window.location.origin}/api/oidc/token`);
        }
      }
    ...
    try {
      ...
      if (config?.showDebugInformation) {
        ...
        if (this.oauthService.hasValidAccessToken() || this.oauthService.hasValidIdToken()) {
          ...
          if (accessToken) {
            console.log('[AuthService] Access Token (first 20 chars):', accessToken.substring(0, 20) + '...');
            console.log('[AuthService] Token expiration:', new Date(this.oauthService.getAccessTokenExpiration()));
          }
          if (idToken) {
            console.log('[AuthService] ID Token (first 20 chars):', idToken.substring(0, 20) + '...');
          }
          console.log('[AuthService] Identity Claims:', this.sanitizeClaims(this.oauthService.getIdentityClaims()));
        } else {
          ...
          console.log('[AuthService] SessionStorage keys:', Object.keys(sessionStorage));
        }
      }
  ...
  private setupOAuthEvents(): void {
    ...
    this.oauthService.events
      ...
      .subscribe(() => {
        const config = this.oidcConfigService.getConfig();
        if (config?.showDebugInformation) {
          console.log('[AuthService] Token received event - loading user profile');
        }
        this.loadUserProfile();
      });
  ...
  private loadUserProfile(): void {
    ...
    if (config?.showDebugInformation) {
      console.log('[AuthService] Loading user profile from claims:', this.sanitizeClaims(claims));
    }
    ...
    if (claims) {
      ...
      if (claims['apps']) {
        if (Array.isArray(claims['apps'])) {
          ...
          apps = claims['apps'].split(',').map(app => app.trim()).filter(app => app);
        }
      }
      ...
      if (config?.showDebugInformation) {
        console.log('[AuthService] Current user after set:', this.currentUser());
